...
class smPurk:
    ...
    #Dend coordinate
        self.sectioncoordinate = np.genfromtxt("smallcoord.csv")
    ...
    #Connection between each dend    
        for c in np.genfromtxt("smallconn.csv"):
            self.dend[int(c[0])].connect(self.dend[int(c[2])],int(c[3]),int(c[1]))

    
        self.subsets = np.genfromtxt("smallsubset.txt", dtype = int)
        ...
        self.subsets_cm = np.genfromtxt("smallsubset_cm.txt")
        for cm in self.subsets_cm:
            for d in self.ModelViewParmSubset[int(cm[0])]:
                d.cm = cm[1] * 0.77/1.64
    
        if len(self.dend)>138:
            self.dend[138].cm = 8.58298 * 0.77/1.64
    
        self.subsets_paraextra = np.genfromtxt("small_extra.txt", dtype=[('modelviewsubset','f8'),('channel','S5'),('channel2','S5'),('value','f8')])
    ...
    def getmaxlength(self):
        ...
        for sec in self.dend:
            if h.distance(self.soma(0),sec(1))>maxlength:
                maxlength=h.distance(self.soma(0),sec(1))
        print("maxlength=",maxlength)  
        return maxlength          

    def alldendlength(self):
        h.define_shape()
        L=0
        for sec in self.dend:
            L+=sec.L
...
class Purkinje:
    ...
    def prune(self,cutoff=300):
        ...
        oldi=[]
        remi=[]
        flist = open("small.dlist", "w")
        for i, sec in enumerate(self.dend):
            if h.distance(self.soma(0),sec(0.5))>cutoff:
                remi.append(i)
            else:
                # print name to dlist file
                flist.write(str(sec))
                flist.write("\n")
                # print that line (+1) from coord file to smallcoord file, with new index number if applicable
                k+=1
                oldi.append(i)
        print("disconnected this number of sections =",len(remi))   

        # remove all lines from conn file that contain values of i in remi (in first and third columns)
        # convert oldi values to newi values (first and third columns)
        array = list( csv.reader( open( r'./connections.csv' ), delimiter=' ' ) )
        connarray=[]
        for x,row in enumerate(array):
            connarray.append([int(array[x][0]), int(array[x][1]), int(array[x][2]), float(array[x][3])])

        connarray=np.array(connarray)
        for val in remi:
            connarray=connarray[(connarray[:,0]!=val) & (connarray[:,2]!=val)]

        # remove all lines from conn file that contain values of i in remi (in first and third columns)
        # convert oldi values to newi values (first and third columns)

        swap=connarray[:,0]
        otherswap=connarray[:,2]
        for i, val in enumerate(oldi):
            swap[swap==val]=i
            otherswap[otherswap==val]=i
        connarray[:,0]=swap
        connarray[:,2]=otherswap
        np.savetxt("smallconn.csv", connarray, delimiter=" ", fmt= ["%d", "%.4f",  "%d",  "%.4f"])

        # read in coord file
        array = list( csv.reader( open( r'./coordinate.csv' ), delimiter=' ' ) )
        coordarray=[]
        for x,row in enumerate(array):
            coordarray.append([int(array[x][1]), float(array[x][2]), float(array[x][3]), float(array[x][4]), float(array[x][5]), float(array[x][6]), float(array[x][7]), float(array[x][8]), float(array[x][9])])

        coordarray=np.array(coordarray)
        for val in remi:
            coordarray=coordarray[coordarray[:,0]!=val]
        swap=coordarray[:,0]
        for i, val in enumerate(oldi):
            swap[swap==val]=i
        coordarray[:,0]=swap
        np.savetxt("smallcoord.csv", coordarray, delimiter=" ", fmt= "%i %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f")

        # read in model subset file
        array = list( csv.reader( open( r'./ModelViewParmSubset.txt' ), delimiter=' ' ) )
        subarray=[]
        for x,row in enumerate(array):
            subarray.append([int(array[x][0]), int(array[x][1])])

        subarray=np.array(subarray)
        for val in remi:
            subarray=subarray[subarray[:,0]!=val]
        swap=subarray[:,0]
        for i, val in enumerate(oldi):
            swap[swap==val]=i
        subarray[:,0]=swap
        np.savetxt("smallsubset.txt", subarray, delimiter=" ", fmt= ["%d", "%d"])

        # read in model subset cm file
        array = list( csv.reader( open( r'./ModelViewParmSubset_cm.txt' ), delimiter=' ' ) )
        subarray=[]
        for x,row in enumerate(array):
            subarray.append([int(array[x][0]), float(array[x][1])])
        ...
        for i, val in enumerate(oldi):
            swap[swap==val]=i
        subarray[:,0]=swap
        np.savetxt("smallsubset_cm.txt", subarray, delimiter=" ", fmt= ["%d", "%.4f"])

        # read in extra model subset file
        array = list( csv.reader( open( r'./modelsubsetextra.txt' ), delimiter=' ' ) )

        subarray=[]
        for x,row in enumerate(array):
            subarray.append([array[x][0], array[x][1], array[x][2], array[x][3]])

        subarray=np.array(subarray)
        for val in remi:
            subarray=subarray[subarray[:,0]!=str(val)]
        swap=subarray[:,0]
        for i, val in enumerate(oldi):
            swap[swap==str(val)]=str(i)
        subarray[:,0]=swap
        np.savetxt("small_extra.txt", subarray, delimiter=" ", fmt= ["%s", "%s", "%s", "%s"])
        #np.savetxt("small_extra.txt", subarray, delimiter=" ", fmt= ["%d", "%s", "%s", "%E"])